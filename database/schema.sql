...
-- Updated for maintenance routine system with enhanced features
...
DROP INDEX IF EXISTS idx_maintenance_routines_user_id;
DROP INDEX IF EXISTS idx_maintenance_routines_category;
DROP INDEX IF EXISTS idx_maintenance_routines_priority;
DROP INDEX IF EXISTS idx_maintenance_routines_is_active;
DROP INDEX IF EXISTS idx_routine_instances_routine_id;
DROP INDEX IF EXISTS idx_routine_instances_due_date;
DROP INDEX IF EXISTS idx_routine_instances_is_completed;
DROP INDEX IF EXISTS idx_routine_instances_is_overdue;

-- Create indexes for performance
CREATE INDEX idx_maintenance_routines_user_id ON maintenance_routines(user_id);
CREATE INDEX idx_maintenance_routines_category ON maintenance_routines(category);
CREATE INDEX idx_maintenance_routines_priority ON maintenance_routines(priority);
CREATE INDEX idx_maintenance_routines_is_active ON maintenance_routines(is_active);
CREATE INDEX idx_routine_instances_routine_id ON routine_instances(routine_id);
CREATE INDEX idx_routine_instances_due_date ON routine_instances(due_date);
CREATE INDEX idx_routine_instances_is_completed ON routine_instances(is_completed);
CREATE INDEX idx_routine_instances_is_overdue ON routine_instances(is_overdue);
...
-- Function to automatically create routine instances
CREATE OR REPLACE FUNCTION create_routine_instances()
RETURNS TRIGGER AS $$
DECLARE
  next_date TIMESTAMPTZ;
  instance_count INTEGER := 12; -- Create instances for the next year
BEGIN
  -- Only create instances for active routines
  IF NEW.is_active = false THEN
    RETURN NEW;
  END IF;
  
  -- Create instances starting from the start_date
...
-- Trigger to create instances when a new routine is created
...
RETURNS TRIGGER AS $$
BEGIN
  -- Update overdue status based on due date and completion status
...
-- Function to create next instance when one is completed
...
BEGIN
  -- Only proceed if this instance was just completed
  IF NEW.is_completed = true AND OLD.is_completed = false THEN
    ...
    -- Create next instance if it doesn't exist
    INSERT INTO routine_instances (routine_id, due_date, is_overdue)
...
-- Trigger to create next instance when one is completed
